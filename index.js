...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // 自动播放变量
  const autoPlayTime = 4000; // 4 秒切换一次
  ...
  const clickableArea = document.querySelector(".slider-wrapper"); // 监听轮播图片区域点击
  ...
  prevBtn.addEventListener("click", (e) => {
    e.stopPropagation(); // **关键修复：阻止事件冒泡到父元素 heroImage/clickableArea**
    stopAutoPlay();
    goToSlide(currentIndex - 1);
    startAutoPlay();
  });
